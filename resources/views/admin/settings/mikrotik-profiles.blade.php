...
<div class="pagehead">
    <div><h1>PPPoE Profile</h1><p>Profile yang sudah diambil dari Mikrotik dan nanti dipilih pada form pelanggan.</p></div>
...
<div class="mikrotik-subhero"><span>PPPoE Data</span><b>Profile Mikrotik</b><p>Data profile dipindahkan ke sub menu supaya halaman pengaturan router tetap ringan dan rapi.</p></div>

<div class="mikrotik-sync-panel">
    <div class="mikrotik-sync-head">
        <div><b>Ambil PPPoE Profile</b><span>Membaca daftar profile dari router Mikrotik dan menyimpannya ke database.</span></div>
    ...
    <div class="mikrotik-router-grid">
        @forelse($routers as $router)
            <div class="mikrotik-router-mini">
                ...
                <form method="POST" action="{{ url('/admin/settings/mikrotik/'.$router->id.'/sync-profiles') }}" onsubmit="return confirm('Ambil PPPoE Profile dari router ini?')">
                    @csrf
                    <button class="btn" type="submit">Ambil Profile</button>
                </form>
            </div>
        @empty
...
<div class="network-section-head" style="margin:16px 2px 9px">
    <b>PPPoE Profile Tersimpan</b>
    <span>Profile yang sudah diambil dari Mikrotik dan nanti akan dipilih pada form pelanggan.</span>
</div>

<div class="neo-xls">
    <div class="neo-xls-info">
        <span>Total profile: <b>{{ \App\Models\MikrotikPppoeProfile::count() }}</b></span>
    ...
    <div class="neo-xls-scroll">
        <table class="neo-xls-table">
            ...
            <tbody>
                @php($profiles = \App\Models\MikrotikPppoeProfile::with('router')->orderBy('name')->limit(50)->get())
                @forelse($profiles as $profile)
                    <tr>
                        <td>{{ $profile->router?->name ?: '-' }}</td>
                        <td class="neo-strong">{{ $profile->name }}</td>
                        <td>{{ $profile->local_address ?: '-' }}</td>
                        <td>{{ $profile->remote_address ?: '-' }}</td>
                        <td>{{ $profile->rate_limit ?: '-' }}</td>
                        <td>{{ $profile->only_one ?: '-' }}</td>
                        <td>{{ $profile->last_synced_at?->format('d/m/Y H:i') ?: '-' }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7">Belum ada PPPoE profile. Klik tombol Profile pada router untuk mengambil data.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
